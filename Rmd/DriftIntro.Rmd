---
title: <center>Thinking About Finite Populations</center>
output:
  html_document:
    css: ~/Chapter.css
    toc: yes
...
Let's look at some real data.  In the human data we looked previously, we saw that departures from Hardy Weinberg were minimal.  Let's now look at another example, some microsatellite data from the legume [Medicago trunculata](http://www.biomedcentral.com/1471-2229/6/28).  The data file is available on Canvas; we will walk through the following steps in Genalex:

1.  We will look at departure from Hardy Weinberg
2.  We will use the population frequency tool to look at heterozygosity.

And what we see is

1.  Lots of deviation from HW.
2.  A severe heterozygote deficiency
...
```
And what we see is that while initially all populations were heterozygous, by the end, all are fixed for one of the two alleles.  So does this happen in real life?  In 1956, [Buri] performed just this experiment, using populations of *Drosophila melanogaster* that were initially heterozygous for two alleles of the *bw* locus, one that affects eye color.   His results are shown below
