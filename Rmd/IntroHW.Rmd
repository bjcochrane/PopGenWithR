...
So let's approach this as a problem in data analysis.  The following are data for the presence(+) or absence (-) pf an insertion in the &delta;-32 gene in human populations (one that has been associated with resistance to HIV infection)
...
1.  What are the allele frequencies (i. e. how many +'s and how many -'s are there)?
...
Now, let's ask how many +'s there are.  Recognizing that every + + homozygote  has two and every + - heterozygote has one, we can calculate that by
```{r}
nA <-2*obs[1]+obs[2]
nA
```
...
```
Now, we could repeat that process to determine the frequency of -, but here one of the simplest but most important relationships comes in to play.  Note that this is a biallelic locus, so if a particular allele is not A, it has to be G.  Thus, if we set the frequency of G to be q, then p+q must equal 1, or
```{r}
q <-1-p
q
```
