...
```
Now, we could repeat that process to determine the frequency of -, but here one of the simplest but most important relationships comes in to play.  Note that this is a biallelic locus, so if a particular allele is not G, it has to be T.  Thus, if we set the frequency of T to be q, then p+q must equal 1, or
```{r}
q <-1-p
q
```
...
p{GG} = p{G}*p{G} = p^2^  
p{TT} = p{T}*p{T} = q^2^  
p{GT} =p{GT} or p{TG} = P{G} * P{T} + P{T} *P{G} = pq + qp = 2pq
