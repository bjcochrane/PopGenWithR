...
What we want to do is to find a tree that requires the fewest changes.  We won't delve into methodology here; the important point is that we will attempt to find the tree that requires the fewest mutational changes, or the most parsimonious tree.  The easiest way to do that is with a function in the package phangorn (which may need to be installed in the usual way)
```{r, eval=FALSE}
...
```
Don't take this tree too seriously; it's here to make the point that while we can construct a phylogeny of our 17 sequences  (and in fact rearrange it, argue over its validity, etc).  But remember - we are dealing with alleles in a population, not genes in different species.  So we need to dig further.
...
#### Plotting the trees
We will talk a lot more about using ms output, but for now, we can use the read.tree function in ape to extract the data and then plot the trees
