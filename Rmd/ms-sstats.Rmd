---
title: 
- <center>Combining Data and Simulation</center>
output:
  html_document:
    css: Chapter.css
    toc: yes
...
As has been mentioned previously, ms and related programs were originally written to be standalone programs, run from the command line in a Unix system.  Thus, they can be used in that way (or with the system() function in R) on Mac's or in most versions of UNIX/Linux.  Unfortunately, they are less compatible with Windows.  However, ms (but not other associated programs) has been implemented as a function in the phyclust R package, so whenever possible, we will work in that environment
...
#ss.ms <-read.ms.output(txt=ss.ms)
```
Now we will use the function ms.sumstats (part of TeachingPopGen) to calculate summary statistics on all 1000 simulations
```{r}
...
```{r}
colhist(ss.sum$Tajima.D,labs=c("D","Number","Distribution of Tajima Statistic"))
...
1.  The expectation, shown by the red line, is close to zero - not surprising (the reason it is not zero is not important to us)
2.  Our observed value of D, `r D$D`, shown by the green line, is significantly different from the expectation, in the negative direction.  This suggests, as we've mentioned before, that in these data, there are an excess of singletons, but how we can explain that remains a problem.
...
And we get the typical kind of output.  So let's first look graphically at the distribution of segregaring sites:
```{r}
hist(ms.sum$S)
```
...
Finally, we really want to address what forces - demographic, selection, etc.  - might underlie any departure from expectation.  To do that, we're going to look at only one example as a means of framing the problem we face.  Suppose we use our results so far to set theta to be 12 (the value estimated from the Bayesian analysis), and let's compare two demographic models - one with constant population size, and one in which the population has been undergoing exponential growth.
...
```{r}
set.seed(4321)
tr.const <- ms(nsam=10, nreps=1, opts=("-t 12 -r 0 702 -T"))
```
...
```{r}
m.out <-ms(nsam=10, nreps=1, opts=(paste("-t 12 -T -r 0 702 -G",g,sep=" ")))
...
tr.const <-read.tree(text=tr.const)
...
Let's consider our original data, with 42 segregating sites, and look at the expected distribution of D under this scenario, using the value of theta we got from our Bayesian estimation:
```{r}
ms.gr <-ms(nsam=42,nreps=1000,opts=(paste("-t",pmean, "-r 0 702 -G",g,sep=" ")))
...
And what we see is as expected. The mean value for D is `r Dg.mean`, which is less than zero.  Just to complete the story, we can do a rough histogram of the simulated values

```{r}
par(mfrow=c(1,1))
d.gr <-growth.sum$Tajima.D[!is.na(growth.sum$Tajima.D)]
colhist(d.gr,labs=c("D","Number","Distribution of D, Growth Model"))
Dg.mean <-mean(growth.sum$Tajima.D,na.rm=TRUE)
...
#### An Aside on the Fay Wu Statistic

We did not calculate this for the spaghetti squash data.  To do so would require us to distinguish between ancestral and derived bases, something we could do but which would require a fair amount of data manipulation.  However, since we have calculated it for our simulated data (both with and without growth), it would be interesting to compare the two:
```{r}
par(mfrow=c(2,1))
hist(ss.sum$H,main="Constant")
abline(v=mean(ss.sum$H),col="red")
hist(growth.sum$H, main="Growth")
abline(v=mean(growth.sum$H),col="red")
```
And in both cases, we see that the mean value is close to zero.  This is to be expected, since in the absence of selection, the ancestral and derived alleles are expected to have similar histories.
