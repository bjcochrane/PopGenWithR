...
### Analytical solution to decay of F.

So what we see is a relationship we have 
1.  Assume population consists of indivs ibd (homozygous) and not - F and 1-F
...
lines(f100,col="red")
```
The fist is not very good.  What if we knew the population history and used the mean?
```{r}
nmean <-mean(N)
nmean
fmean <-fexp(nmean)
