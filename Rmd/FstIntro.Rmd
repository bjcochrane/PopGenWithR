...
```{r}
set.seed(1123)
pop1 <-HWData(nm=1,n=100,f=runif(1,0,1), p=runif(1,.05,.5))$Xc # generate data for a random locus with some p between .05 and .5 and some f between 0 and 1
pop1
```
...
Hexp
```
And of course, Hobs is the observed frequency of AB, the second element in pop1, or `r pop1[2]` in this case.  So we can get F as follows
```{r}
F.pop1 <-1-(pop1[2]/Hexp)
F.pop1
```
...
Remember the problem 3 from Chapter one in Felsenstein.  In that problem. we had two populations with the following allele frequencies
```{r}
pop1 <-c(.64,.32,.04)
pop2 <-c(.09,.42,.49)
```
We saw that both were in Hardy-Weinberg equilibrium, so in the context of our current consideration
...
popt
```
We can now look at the observed and expected heterozygosity in the combined sample, using a simple function to calculate F, using a simple function in the TeachingPopGen package.
```{r}
f.comb <-Fis(popt)
f.comb
```
...
```{r}
dat <-(sapply(c(1:5),function(x) (HWData(nm=1,n=100,f=runif(1,.05,.2),p=runif(1,.1,.9))$Xc)))
