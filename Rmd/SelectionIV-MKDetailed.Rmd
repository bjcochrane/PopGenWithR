---
title: "Using Genomic Data to Detect Selection"
...
```
So we see what looks like an overall pattern of departure from neutral expectation, suggesting that in general, the N:S ratio is higher for polymorphism than for divergence.  Balancing selection??  Not so fast.
...
```{r}
MK(c(130,250),c(60,180))
```

And we get a significant result, with FI>1, implying (as in the case of the Adh example) that there may have been positive selection giving rise to substitutions.  We know, however, that this is actually an artifact, due to the fact that the second locus is more constrained with respect to divergence (total substitutions=200 vs. 180) but more polymorphic (180 vs. 60).  Thus, there is a negative correlation between A/S and P/D. So what we need to do is the following:
...
S <-(c(2411,6286))
Obs <-rbind(N,S)
rownames(Obs) <-c("Nonsynonymous","Synonymous")
colnames(Obs) <- c("Polymorphism","Divergence")
Obs
```
...
1.  Constraint - limit (due to selection) on permissible substitutions - if high, the ratio of nonsynonymous to synonymous *divergence* should be low.
...
plot(ratio.NS, ratio.PD,ylim =c(0,5),xlab="Selective Constraint (Pn/Ps)",ylab="Polymorphism/Divergence")
```
...
summary(reg)
plot(ratio.NS, ratio.PD,ylim =c(0,5),xlab="Pn/Ps",ylab="Polymorphism/Divergence")
abline(reg,col="red")
#abline(a=reg$coefficients[1],b=reg$coefficients[2],col="darkred")
```
What we see is that indeed the more constrained loci (with low values of Pn/Ps) do indeed tend to be more polymorphic, relative to their total divergence.   
...
Note that the first three columns (derived alleles with 1, 2, and 3 copies in the sample) have FI's higher than the rest, suggesting that,  there is a lower level of nonsynonymous polymorphism relative to synonymous because of purifying selection acting against these alleles.  based on this, we can first calculate the total ratios for these two classes:
```{r}
as.tot <-1462/4695
as.low <-450/2413
```
...
Discriminating between these two possibilities is difficult.  However, Shapiro et al divided their data into those genes that occur in regions with normal levels of recombination and those in recombinationally constrained regions, and doing a quick test of the aggregated data


```{r}
high.r <-MK(c(1315,297),c(3442,1883))
low.r <-MK(c(992,152),c(481,528))
high.r; low.r
```
...
### Supplemental code on recombination data

Create matrices of data
```{r}
obs.hr <-rbind(c(297,1315),c(1883,3441)) 
obs.lr <-rbind(c(152,992),c(528,2844))
```
Now bootstrap one of them
```{r}
boot.lr <-boot(obs.lr)
```
And pull out the FI's and plot
```{r}
FI.lr <-sapply(boot.lr,FI)
hist(FI.lr)
mean(FI.lr)
```
Now, what was FI.hr?
...
FI(obs.lr)
```
Here,s and idea.  Can we apply our bootstrap function to all of the loci?.  We would need to 

1.  create a df of the relevant columns.
2.  add 1 to Ds and Pn
3.  Change FI function back to original
4.  do an apply to generate a list of matrices
5.  apply boot to the results
6.  Apply FI and plot
