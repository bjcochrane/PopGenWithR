---
title: <center>The Basics</center>
...
In fact, there are many more elements that can be examined; we are going to restrict most of our attention (as do Nielsen and Slatkin) to viability as a measure of fitness, briefly considering fertility as somewhat of a special case.
...
Consider the example described in box 7.2 of Nielsen and Slatkin.  In this case, we are looking at a mouse coat color polymorphism, with two alleles, C and R:
...
N.post
```
Note a couple of things here.  First, whereas initially there were `r N` embryos, after selection, there are only `r N.post` adults.  If we take the ratio of those two values, we get the **mean fitness** of the population
```{r}
wbar <-N.post/N
wbar
```
...
The following R code implements that equation.  It takes as inputs the initial value of p, the falues of h and s, and whether or not a plot is to be returned.  The default function returns a vector with the values of p.
```{r}
...
```
And we see that, as expected, while q declines, is does so much more slowly.  But now let's add in selection against the heterozygote as well, setting h to .1.
```{r}
...
Now let us suppose that s=.2, as above
...
And as an example, suppose that s=.4 and t=.3.  To use our function, we need to do some algebra to normalize relative fitnesses to that of AA, but when we do, we find that s=.166 and h=-4.125, so we can plug those into our function and we get

```{r}
deltap(.1,-4.125,.16,plot=TRUE)
abline(h=0,col="red")
```
