...
genos.f <-genos.n/sum(genos.n)
p <-genos.f[1]+.5*genos.f[2]
genos.exp <-c(p^2,2*p*(1-p),(1-p)^2)
genos.f; genos.exp
```

#### Make some assumptions

1.  **At the time of fertilization** the population is at equilibrium.
...
As an example, we can look at some reasonable numbers - µ=10^-6, s=.01
```{r}
u <-10^-7
s <-.01
q1 <-sqrt(u/s)
...
```
But let's take this one step further.  Suppose it does survive and increase in frequency as a result of selection.  How many alleles would we expect to observe in an infinite population?  This is simply (1+s)^t, however this is average of distribution including both those populations that have retained it as well as lost it.  So consider with s=.01 and 1000 generations
```{r}
pret=p1[2] # probability of survival 
mean.alleles <-(1+.01)^1000 # given survival, number of alleles after 1000 generations
mean.alleles
```
BUT - what is probability that a given population has lost the allele?
```{r}
1-pret
```
...
Now, what if we have a finite Ne?  What we want to do is to ask what's the probability of fixation? For a neutral allele, that probability is 4N, but what about when we add in s?  The math is far from trivial (see Felsenstein pp. 253-258 for details), but the key result is in the following equation, which gives the probability of fixation of an allele with a **positive** selection coefficient of s and a population size of N:
...
blank <- function() plot(0,bty='n',pch='',xlab='p',ylab='U', xlim =c(0,1),ylim=c(0,1))
```
...
But remember - our problem is to explain observed levels of heterozygosity. Again, we won't grind through the math,  but we can calculate the ratio of Het(Ns)/Het(neutral) by
```{r}
...
1.  In large populations, selection will predominate and their frequency will remain low.
2.  In small populations, even though the fixation probability is low, heterozygosity can be high

Thus, one can predict that there is an upper level on heterozygosity for weakly deleterious alleles, determined by selection in the case of large populations and drift in the case of small ones. This was a key argument that was used to address the criticism that selective neutrality could not explain the comparable levels of heterozygosity observed in widely disparate species.
