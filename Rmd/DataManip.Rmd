...
To fix all of this, we could do a bunch more manipulations.  However, for our current purposes, the goal of this unit is to get data we can work with, not to explore the vagaries of data munging.  So, let's load a function I wrote for just this purpose:

```{r}
source("ALFRED.R")
```
Notice that now a bunch of functons have shown up in the workspace.  What we are interested now is the read.alfred function.  Note that it has "the.url" as an argument.  So, what we need to do is to provide it with the complete url that we created above.

```{r}
url2 <-paste(url, siteid, sep="")
dat <-read.alfred(url2)
```
