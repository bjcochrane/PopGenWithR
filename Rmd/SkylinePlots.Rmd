...
```
Note that what we see are stepwise changes - obviously an approximation.  And when we find the minimum interval for which noise is filtered out, we see that what was in fact a continuou process gives a signal of a single stepwise one.
...
```
And we wee a pretty good indicator of a bottleneck followed by growth.  
...
The stepwise model we have been using so far, one that assumes population changes are associated with coalescenc events, is clearly not biologically realistic.  THis method, implemented in APE, derives a smooth population size function based on the given geneologies.  The paper describing the work is quite straightforward (at least as theoretical papers go) and is worthy of at least brief perusal  [Opgen-Rhein 
et al (2005)](http://www.biomedcentral.com/1471-2148/5/6) developed methodology for "smoothing" these curves - in essence it takes an MCMC approach to find a set of paramters that results in a smooth population growth function based on one or more gene geneologies.
...
Hoffmann et al have reported mitochondrial DNA for fur seals from the North Atlantic.  We will look more closely at their data when we introduce BEAST;   however, we can do a little bit of rough analysis with what we've done so far
...
```
Based on that, we can then do our MCMC smoothing
...
What we see is that there is clear evidence for an historically stable population, but based on this modeling, we do not see the significant decline that appears in the generalized skyline plot.  
...
```{r}
par(mfrow=c(1,1))
plot(skyline(tr.fur.u,eps.fur),show.years=TRUE, subst.rate=Mu,present.year=2010,ylim=c(.002,1.2))
lines(fur.pop,col="red",show.years=TRUE,subst.rate=Mu,present.year=2010)
```
...
What about spaghetti squash as a substitute for fur?
```{r}
data(spaghetti)
spaghetti
```
...
```
At this point, one would need to do some more investigation, asking such questions as

1.  Do we have any estimate of mutation rate?
2.  What do we know about the past history of this population?
