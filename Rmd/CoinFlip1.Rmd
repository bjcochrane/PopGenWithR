...
In the simplest case, working with genetic data is an exercise in the manipulation of binomial (or in the multi-allelic case multinomial) random variables.  Typically, one could start with the example of a coin flip,consider combinations and permutations, and then apply the binomial theorem.  Alternatively, the same result can be obtained via simulation, and in a way that leads more directly to dealing with more complex situations.
...
Thus, let's ask the question,  given our simulated results, which outcomes fall in that category.  In otherwords, of the 10000 simulations of 20 coin flips, which gave outcomes that occurred less than 5% of the time.  the R quantile () function does that; as implemented below, it finds the two tails of the distribution 
```{r}
...
colhist(flips,tail=2,xr=c(0,20),labs=c("# Heads","# Ocurrences","Is it a Fair Coin?"))
