---
title: <center>The Fate of New Mutations</center>
...
In his chapter on stochastic processes in evolution, Gillespie (2006) begins with "Pity the fate of the  mutation".  Why?  It is worth exploring this question in some detail.
...
For this, we turn away from our simulation approach.  It turns out that [Fisher, 1930, pp 73-77](https://ia600301.us.archive.org/29/items/geneticaltheoryo031631mbp/geneticaltheoryo031631mbp.pdf) has a pretty straight forward analytical one.  
...
Now what about future generations?  We can set up recursion, in which we start with x=0, and with each generation set x[i+1] to e^c(x[i]-1)^.  So to get x[2, for example], we could do the following:
```{r}
x2 <-exp(x1-1)
x2
```
