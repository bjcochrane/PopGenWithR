...
In our previous unit, we saw how we could quantify population subdivision based on variation in allele frequencies among subpopulations. However, we only considered the simplest possible case, one in which 

In 2010, `r citet ("10.1111/j.1365-294X.2010.04541.x")` published a wonderful paper on genetic variation in Idaho Giant Salamanders.  This is the creature:
...
```
Now, we will do our nested F statistic analysis of the data. We will load the library hierfstat and use the function varcomp.glob() to do so.  The syntax may look a bit arcane, but it does make some sense.  What we are telling it to do is to apply our three levels (basin, catchment, stream) in that order to the data frame containing the genetic data (loci) and do all of the calculations:
