...
res <-data.frame(c("Caucasian","Maori","W.Polynesian"),c(ps.c,ps.m,ps.wp))
colnames(res) <-c("Ethnicity","Probability")
printx(res)
```
...
colhist(post,xr=c(0,1),labs=c("p","Number","Posterior Distribution, # Heads = 8"))
mean(post)
...
```{r}
par(mfrow=c(1,2))
colhist(post,labs=c("P","Frequency","Initial Posterior Distribution"))
colhist(post1,labs=c("P","Frequency","Final Posterior Distribution"),xr=c(0,1)) 
```
