...
```
We then need the *likelihoods* that the particular genotype would be found in these populations (presumably done by some prior population-based genotyping).  These are essentially conditional probabilities, for example "what is the probability of getting the genotype, given that the individual is a Maori"?
```{r}
l.c <-3.96*10^-9
l.m <-1.18*10^-8
l.wp <-1.91*10^-7
...
But note that this is the reverse of what we want.  We want to know the probability that an individual is, for example, a Maori, given it has the genotype.  To do this we proceed as follows.  First, we can calculate the total probability that this blood sample would be found in this population (p{X}) by multiplying each likelihood by the frequency of the three ethnic groups  (the *prior distribution*) and summing:
...
Not an easy experiment to do in real life, but simulating it is straightforward.  Let's suppose we do a million coin tosses, and for each one, there is a uniform probability of a head that is some value between zero and one (this is considered and *uninformative prior*).  First we can simulate the probabilities:

```{r}
set.seed(12345) # sets random number generator seed so that results are reproducible
...
Remember - we did a single experiment and used an uniformative prior.  However, let's make the problem a bit more complicated.  Suppose that

1.  The true probability of getting a head is actually 0.8, so the outcomes we observe are distributed binomially with p=0.8.
2.  We do 100 coin flips, instead of the single one we did previously, and we refine our use our posterior for each coin flip as the prior for the nex.
...
set.seed(54321)
out.mean <-rep(0,100)
# The following loop performs 50 iterations
for(i in 1:100){
  outcomes <-rbinom(1000000,10,p) # Simulate the coin flips
  out.8 <-rbinom(1,10,.8) # select an outcome that would be obtained assuming p=.8
  post1 <-p[which(outcomes==out.8)] # select the probabilities associated with those outcomes
  out.mean[i] <-mean(post1) #calculate the mean of those probabilities for future use
  p <-sample(post1,1000000,replace=TRUE) # create the new prior distribution
}
```
...
And we see that, as we expected intuitively, it approaches a value of .8.  We can also look at the initial and final posterior distributions of p (after 50 iterations):

```{r}
par(mfrow=c(1,2))
colhist(post,xlab="P",ylab="Frequency",main="Initial")
colhist(post1,xlab="P",ylab="Frequency",main="Final ",xr=c(0,1)) 
```

Two features are evident.  First, as we already saw, the mean of the distribution is very close to .8, which is what we would intuitively expect given that we keep the true probability of a head.  Second, compare the distribution with that we saw when we did the calculation based on our initial uniform prior distribution, we got the following confidence limits:
