...
p <-runif(10000,0,1) # generate 1 million random numbers between 0 and 1
head(p)
```

And we can quickly plot these values as a histogram:

```{r, fig.margin=TRUE,fig.cap="Figure 1.  Distribution of 10,000 random variables distributed between zero and one"}
ggplot(data.frame(dat=p),aes(dat))+
...
```{r}
outcomes <-rbinom(10000,10,p)
head(outcomes)
```
And we can look at the distribution of outcomes we got

```{r,fig.margin=TRUE,fig.cap="Figure 2.  The results of taking each value of p we generated and flipping a coin with that probability of getting a head 10 times."}
ggplot(data.frame(results=outcomes),aes(results))+
...
```{r,fig.margin=TRUE, fig.cap="Figure 3.  Posterior Distribution of p, given that the outome of the experiment (the data) was 8 out of 10 heads."}
m <-mean(post)
m
...
ggplot(data.frame(p=post),aes(p)) +
  ...
  geom_histogram(bins=40, fill="red", alpha=.5, data=subset(data.frame(p=post),p<q[1]|p>q[2]))
...
```{r, fig.margin=TRUE,fig.cap="Figure 4.  Our estimate of p as a function of the number of trials with the outcome of eight heads.  Notice that the value oscillates initially, but it stablizes after about 35-40 trials"}
ggplot(data.frame(Iteration =1:100, p=out.mean), aes(Iteration, p)) +
  ylim(0.5,1) + 
         geom_line()
```                    
...
```{r,fig.margin=TRUE,fig.cap="Figure 5.  Posterior Distribution of p based on a single trial with the outcome of eight heads"}
pp1 <-ggcolhist(post) +
  xlim(0,1) +
  labs(x="p") 
pp1
...
```{r,fig.margin=TRUE, fig.cap="Figure 6.  Posterior distribution of p based on 100 trials, each with an outcome of 10 heads."}
pp2 <-ggcolhist(post1) +
  xlim(0,1) +
  labs(x="p")
pp2
