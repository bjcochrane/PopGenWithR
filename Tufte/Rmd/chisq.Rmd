---
title:  "A More General Approach </br>The &chi;^2^ Distribution"
...
```{r}
set.seed(123)
ch <- data.frame(chi_squared=rchisq(100000,3))
```
...
```{r,fig.margin=TRUE, fig.cap="The distribution of 100,000 values of chi-squared, with 3 degrees of freedom.  The red area indicates the upper 5% of the distribution; the blue line indicates the value of chi-squared calculated from Mendel's round/wrinkled F~2~ data"}
ggplot(ch,aes(chi_squared)) +
  geom_histogram(fill="darkgrey",binwidth=1) +
  geom_histogram(binwidth=1,fill="red",alpha=.5, data=subset(ch,chi_squared>q)) +
  geom_vline(xintercept=chi,color="blue")
```
...
kable(data.frame(cbind(df, chicrit.05=round(chicrit.05,2),chicrit.01=round(chicrit.01,2))))
```
