...
flips <-data.frame(Outcomes=rbinom(10000,20,.5)) # do 1000 replications of 20 flips of a fair coin
length(flips$Outcomes)
head(flips$Outcomes)
...
his1 <-ggplot(flips,aes(Outcomes)) +
  geom_histogram(binwidth=1,fill="darkgrey") +
  ggtitle("10000 Replicates of 20 Coin Flips") +
  labs(x="Number of Heads", y="Number of Occurrences")
his1
```
...
```{r}
Obs17 <-length(which(flips$Outcomes==17))
Obs17
```
...
q <-quantile(flips$Outcomes,c(.025,.975))
q
```
So what this says is that 2.5% of the trials resulted in fewer than 6 heads, while 2.5% resulted in more than 14.  And of course, we observed 18.  We can illustrate this graphically, by modifying the plot we made previously.  In the code below, `geom_hist` colors the tails red, and `geom_abline` draws a vertical line to show the position of our 17 head outcome.


```{r,fig.margin=TRUE}
his1 + 
  geom_histogram( binwidth = 1, fill="red",data=subset(flips,Outcomes <q[1]|Outcomes>q[2])) +
