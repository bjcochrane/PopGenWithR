...
```{r, fig.margin=TRUE}
x <- data.frame(Height=seq(100,260,length=100))   #set a range of possible heights
hx <-dnorm(x$Height,180,30) # calculates the density function, or theoretical exact probability, for every value of x, assuming mean of 180 and standard deviation of 30
x <- cbind(x, Frequency=hx)
#plot(x,hx, type="l",xlab="Height (cm)", main="Hypothetical Distribution of Human Height",yaxt="n",ylab="Frequency")
#abline(v=180,col="red")
ggplot(x,aes(Height,Frequency)) +
  geom_line()
```
...
```{r}
set.seed(123)
runners <-data.frame(Weight=rnorm(1000,163,10))
his <- ggplot(runners, aes(Weight)) +
  geom_histogram(aes(y=..density..),col="darkgrey",binwidth=3) +
  stat_function(fun = dnorm, args = list(mean=mean(runners$Weight), sd = sd(runners$Weight)), 
                col = 'red') +
  geom_vline(xintercept = 185, col="blue")
his
```
...
```{r}
q <- as.integer(quantile(runners$Weight, c(.025,.975)))
q
```
And we can do this on a version of the plot
```{r}
his+
  geom_histogram(aes(y=..density..),binwidth=2,fill="red",data=subset(runners,Weight <q[1]|Weight>q[2]))
colhist(runners,,tail=2,xr=c(140,190),xlab="Weight",ylab="Number",main="Is the Mystery Person a Runner?")
abline(v=185,col="blue")
```
Note to self - all of this needs work.  Scratch follow
```{r}
hs2 <- ggplot(runners, aes(Weight)) +
  geom_histogram(binwidth = 2) 
hs2
hs2 <- hs2 + 
  geom_density(aes(x=Weight, y=..count..*2)) +
  geom_vline(xintercept = 185, col="blue")
hs2
hs2 +
  geom_histogram(binwidth=2,fill="red",data=subset(runners,Weight <q[1]|Weight>q[2]),alpha=.5) 

```

And all is good.  Need to clean it up and integrate tomorrow.
