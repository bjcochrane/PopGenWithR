...
```{r,fig.margin=TRUE, fig.cap="Dashed lines show the approximate position of perpendicular axes onto which the data can be mapped"}
...
```{r,fig.margin=TRUE, fig.cap="Normalized data have been remapped on those axes.  Arrows indicate the loadings, or the magnitude of the contribution of each variable to each axis"}
...
```{r,fig.margin=TRUE,fig.cap="The three-variable data set, plotted in three dimensions"}
scatterplot3d(dat.samp3[,2:4])
```

And now, how can we make any sense of it?  This is where PCA begins to show its usefulness:

```{r,fig.margin=TRUE, fig.cap="PCA of the three variable data"}
samp3.pr <-prcomp(dat.samp3[,2:4])
ggord(samp3.pr,obslab=TRUE)
summary(samp3.pr)
```
...
<iframe width="50%" height="480" src="https://www.youtube.com/embed/ipyxSYXgzjQ" frameborder="0" allowfullscreen></iframe>

# Some real data

But enough of hypotheticals.  Let's see what we can do with genotype data, using the Idaho Giant Salamander (@Mullen2010) data as our source:
...
ggord(basin.pca$ipca,dat.basin$Basin,size=2,vec_ext=0,txt=NULL,arrow=0)
```
...
`
```{r}
ggord(basin.pca$ipca,IGS$Catchment,size=2,vec_ext=0,txt=NULL,arrow=0,ellipse=TRUE)
```
...
```{r}
ggord(basin.pca$ipca,IGS$Catchment,size=2,vec_ext=0,txt=NULL,arrow=0,ellipse=TRUE,axes=c("1","3"))
```
...
```{r}
load("../../Data/SampledSNPs.gen.rda") #In TeachingPopGen
data("poplist")
dat.gen
```
...
```{r}
ggord(dat.pca$ipca,poplist.1000g$Superpopulation.code,vec_ext=0,txt=NULL,arrow=0,ellipse=TRUE,size=1,axes=c("1","3"))
```
