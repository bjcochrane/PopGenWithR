...
```{r,fig.margin=TRUE,fig.cap="A nearly meaningless tree"}
...
```{r, results='hide', fig.margin=TRUE,fig.cap="Six simulated gene geneaologies, all based on the same assumptions."}
#par(mfrow=c(3,2))
trees <-read.tree(text=tr)
names(trees) <-paste0 ("Tree ", 1:6)
#sapply(trees,plot)
#par(mfrow=c(1,1))
ggtree(trees) +
  geom_tiplab(size=2) +
